...
def create_elegant_cv(data):
    ...
    class ElegantCV(FPDF):
        ...
        def add_header(self, name, title, contact_info):
            ...
            name_clean = clean_text(name) if name else "CURRICULUM VITAE"
            self.cell(210, 9, name_clean.upper(), 0, 1, 'C')
            ...
            if title:
                ...
                self.set_font("Helvetica", "B", 10)
                title_clean = clean_text(title)
                self.cell(210, 5, title_clean, 0, 1, 'C')
            ...
            self.set_color(self.text_light[0], self.text_light[1], self.text_light[2])
            self.set_font("Helvetica", "", 7)
            self.cell(210, 4, contact_text, 0, 1, 'C')
            
            self.set_draw(self.accent[0], self.accent[1], self.accent[2])
            self.set_line_width(0.8)
            self.line(55, 40, 155, 40)
        ...
        def add_section_title(self, title, y_pos):
            ...
            self.set_color(self.primary[0], self.primary[1], self.primary[2])
            self.set_font("Helvetica", "B", 9)
            self.cell(self.main_width, 4.5, title.upper(), 0, 1, 'L')
            self.set_draw(self.accent[0], self.accent[1], self.accent[2])
            self.set_line_width(0.3)
            self.line(self.main_x, y_pos + 5, self.main_x + 30, y_pos + 5)
            return y_pos + 8
        ...
        def add_main_content(self, summary, experience, education, achievements, references):
            ...
            if experience:
                ...
                for exp in experience[:4]:
                    ...
                    self.set_color(self.primary[0], self.primary[1], self.primary[2])
                    self.set_font("Helvetica", "B", 8.5)
                    self.cell(self.main_width, 4, company, 0, 1, 'L')
                    y_pos += 4
                    ...
                    if title_text or date_text:
                        ...
                        if title_text:
                            self.set_color(self.accent[0], self.accent[1], self.accent[2])
                            self.set_font("Helvetica", "B", 7.5)
                            self.cell(self.main_width * 0.6, 3.5, title_text, 0, 0, 'L')
                        if date_text:
                            self.set_color(self.text_light[0], self.text_light[1], self.text_light[2])
                            self.set_font("Helvetica", "I", 6.5)
                            self.cell(self.main_width * 0.4, 3.5, date_text, 0, 1, 'R')
                        y_pos += 3.5
        ...
        def add_page(self):
            ...
            self.set_color(self.primary[0], self.primary[1], self.primary[2])
            self.set_font("Helvetica", "B", 16)
            self.cell(210, 7, name_clean.upper(), 0, 1, 'C')
            self.set_xy(0, 24)
            self.set_color(self.text_light[0], self.text_light[1], self.text_light[2])
            self.set_font("Helvetica", "I", 6.5)
            self.cell(210, 3.5, f"Page {self.page_no()}", 0, 1, 'C')
            self.set_draw(self.accent[0], self.accent[1], self.accent[2])
            self.set_line_width(0.8)
            self.line(55, 30, 155, 30)
...
def create_professional_cv(data):
    ...
    class ProfessionalCV(FPDF):
        ...
        def add_sidebar(self, name, contact_info, skills, languages):
            ...
            title_clean = clean_text(title)
            self.cell(sidebar_width, 3.5, title_clean, 0, 1, 'L')
            
            self.set_draw(self.gold[0], self.gold[1], self.gold[2])
            self.set_line_width(0.3)
            self.line(5, 38, sidebar_width, 38)
            
            y_pos = 45
            self.set_xy(5, y_pos)
            self.set_color(self.white[0], self.white[1], self.white[2])
            self.set_font("Helvetica", "B", 6)
            self.cell(sidebar_width, 3.5, "CONTACT", 0, 1, 'L')
            y_pos += 5
            ...
            if contact_info.get('email'):
                ...
                self.set_font("Helvetica", "", 5.5)
                email_clean = clean_text(str(contact_info['email']))
                wrapped_email = textwrap.fill(email_clean, width=14)
                self.multi_cell(sidebar_width, 2.5, wrapped_email, 0, 'L')
                y_pos += len(wrapped_email.split('\n')) * 2.5 + 3
            ...
            if contact_info.get('phone'):
                ...
                self.set_font("Helvetica", "", 5.5)
                phone_clean = clean_text(str(contact_info['phone']))
                self.cell(sidebar_width, 2.5, phone_clean, 0, 1, 'L')
                y_pos += 4
            ...
            if skills:
                ...
                self.set_color(self.white[0], self.white[1], self.white[2])
                self.set_font("Helvetica", "B", 6)
                self.cell(sidebar_width, 3.5, "SKILLS", 0, 1, 'L')
                y_pos += 5
                for skill in skills[:8]:
                    ...
                    self.set_font("Helvetica", "", 5.5)
                    skill_clean = clean_text(skill)
                    self.cell(sidebar_width, 2.5, f"- {skill_clean}", 0, 1, 'L')
                    y_pos += 2.5
            ...
            self.set_color(self.white[0], self.white[1], self.white[2])
            self.set_font("Helvetica", "B", 6)
            self.cell(sidebar_width, 3.5, "LANGUAGES", 0, 1, 'L')
            y_pos += 5
        ...
        def add_main_content(self, summary, experience, education, achievements):
            ...
            if summary:
                ...
                self.set_color(self.primary[0], self.primary[1], self.primary[2])
                self.set_font("Helvetica", "B", 8)
                self.cell(main_width, 3.5, "SUMMARY", 0, 1, 'L')
                self.set_draw(self.accent[0], self.accent[1], self.accent[2])
                self.set_line_width(0.3)
                self.line(main_x, y_pos + 4.5, main_x + 25, y_pos + 4.5)
                y_pos += 6
            ...
            if experience:
                ...
                self.set_color(self.primary[0], self.primary[1], self.primary[2])
                self.set_font("Helvetica", "B", 8)
                self.cell(main_width, 3.5, "EXPERIENCE", 0, 1, 'L')
                self.set_draw(self.accent[0], self.accent[1], self.accent[2])
                self.set_line_width(0.3)
                self.line(main_x, y_pos + 4.5, main_x + 28, y_pos + 4.5)
                y_pos += 6
                ...
                for exp in experience[:4]:
                    ...
                    if exp.get('bullets'):
                        for bullet in exp['bullets'][:2]:
                            ...
                            self.multi_cell(main_width - 10, 3, wrapped_bullet, 0, 'L')
                            y_pos += len(wrapped_bullet.split('\n')) * 3 + 1
                    y_pos += 2
        ...
        def add_page(self):
            ...
            self.set_color(self.white[0], self.white[1], self.white[2])
            self.set_font("Helvetica", "B", 8)
            self.multi_cell(sidebar_width, 3.5, name_clean.upper(), 0, 'L')
            self.set_xy(5, 28)
            self.set_color(self.white[0], self.white[1], self.white[2])
            self.set_font("Helvetica", "", 5)
            self.cell(sidebar_width, 2.5, f"Page {self.page_no()}", 0, 1, 'L')
